...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  function toggleBookmark(p, btn, card) {
    ...
    const player = { name: (p.name || `${p.firstName || ''} ${p.lastName || ''}`).trim(), firstName: p.firstName || '', lastName: p.lastName || '', team: p.team || '', position: p.position || '', height: p.height, weight: p.weight, overall_rating: p.overall_rating, playerClass: p.playerClass || '', cost: computePlayerCost(p) };
    if (wasBookmarked) {
      list.splice(idx, 1);
    } else {
      list.push(player);
